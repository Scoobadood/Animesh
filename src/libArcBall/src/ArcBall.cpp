...
ArcBall::keyPressEvent(QKeyEvent *event) {
  ...
  case Qt::Key_Equal:zoom(-0.5f);
    movedCamera = true;
    break;

  case Qt::Key_Minus:zoom(0.5f);
    movedCamera = true;
    break;
  }
...
ArcBall::compute_ray_through_pixel(unsigned int pixel_x,
                                   ...
                                   QVector2D field_of_view, // Half Fov
                                   float focal_length,
                                   int width,
                                   int height) {
  ...
  double image_plane_height = 2 * tan(fov_rad.y()) * focal_length;
  double image_plane_width = 2 * tan(fov_rad.x()) * focal_length;
