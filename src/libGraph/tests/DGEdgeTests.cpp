...
TEST_F(DirectedGraphEdgeTests, add_duplicate_edge_should_not_increase_edge_count) {
...
TEST_F(DirectedGraphEdgeTests, forward_edge_is_not_retrievable_in_backward_order) {
  ...
  EXPECT_THROW_WITH_MESSAGE(graph->edge(gn2, gn1), std::runtime_error, "No edge from b to a");
}
