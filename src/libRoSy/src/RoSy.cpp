...
best_rosy_vector_pair(const Eigen::Vector3f &target_vector,
  ...
  return best_rosy_vector_pair(target_vector,
                               target_normal,
                               target_k,
                               source_vector,
                               source_normal,
...
best_rosy_vector_pair(const Eigen::Vector3f &o_i,
  ...
  unsigned short best_a = 0;
  unsigned short best_b = 0;
  ...
  const auto dp = A[best_a].dot(B[best_b]);
  auto p = make_pair(A[best_a], B[best_b] * signum(dp));
  if (dp < 0)
    best_b += 2;
