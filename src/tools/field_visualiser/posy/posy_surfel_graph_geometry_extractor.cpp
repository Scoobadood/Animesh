...
void extract_quads_for_frame(const SurfelGraphPtr &graphPtr,
  ...
  for (const auto &node: graphPtr->nodes()) {
    ...
    // Stash the lattice position
    lattice_positions.push_back(reference_lattice_vertex.x());
    lattice_positions.push_back(reference_lattice_vertex.y());
    lattice_positions.push_back(reference_lattice_vertex.z());
...
void posy_surfel_graph_geometry_extractor::extract_geometry(
  ...
  extract_quads_for_frame(graphPtr, get_frame(), positions, lattice_positions, quads, normals, splat_sizes, uvs, m_rho,
